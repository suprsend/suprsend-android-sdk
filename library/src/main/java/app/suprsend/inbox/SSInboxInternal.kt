...
internal class SSInboxInternal {
    ...
    fun loadInitial(
        ...
        try {
//            val storeId = sid ?: (inboxData.notificationStores.firstOrNull()?.notificationStoreConfig?.storeId ?: SSInbox.DEFAULT_STORE_ID)
//            val store = inboxData.notificationStores.find { it.notificationStoreConfig.storeId == storeId }
//            if (notificationStore.hasInitialFetchTime()) {
//                Logger.i(SSInbox.LOGGING_TAG, "Initial page for store:${notificationStore.notificationStoreConfig.storeId} is already loaded")
//                return
//            }
            ...
            loadNextPage(notificationStore)
    ...
    fun loadNextPage(
        ...
        try {
//            val storeId = sid ?: SSInbox.DEFAULT_STORE_ID
...
//            }
            ...
            if (notificationStore.isLoading) {
                Logger.i(SSInbox.LOGGING_TAG, "Loading for store id ${notificationStore.notificationStoreConfig.storeId} is already in-progress ignoring this duplicate call on same store")
                return
            }
            notificationStore.isLoading = true
            notifyLoading(notificationStore.notificationStoreConfig.storeId, notificationStore.isLoading)
            ...
            if (notificationListModel != null) {
                ...
                notifyLoading(storeId = notificationStore.notificationStoreConfig.storeId, isLoading = notificationStore.isLoading)
                notifyListeners(
...
//                    totalPages = notificationStore.totalPages
                ...
                notifyError(notificationStore.notificationStoreConfig.storeId, IllegalStateException("Failed to fetch "))
            }
        } catch (e: Exception) {
            Logger.e(SSInbox.LOGGING_TAG, e)
        }
    ...
    fun markNotificationClicked(storeId: String, notificationId: String) {
        ...
        if (notification.interactedOn == null || notification.seenOn == null) {
            ...
            } else if (notification.seenOn == null) {
...
//    }
    ...
    private fun notificationBelongToStore(notificationModel: NotificationModel, store: NotificationStore): Boolean {
        ...
        return sameRead && sameTags && sameCategory
    }
