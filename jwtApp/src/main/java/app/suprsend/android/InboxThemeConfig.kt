...
data class InboxThemeConfig(
    ...
    val inboxFetchInterval: Long = 15000
) : Parcelable {
    ...
    companion object {
        @JvmField
        val CREATOR: Parcelable.Creator<InboxThemeConfig?> = object : Parcelable.Creator<InboxThemeConfig?> {
            override fun createFromParcel(parcel: Parcel) = InboxThemeConfig(parcel)
            override fun newArray(size: Int): Array<InboxThemeConfig?> = arrayOfNulls(size)
        }
    }
}
