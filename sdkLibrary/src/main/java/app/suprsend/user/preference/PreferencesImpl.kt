...
class PreferencesImpl : Preferences {
    ...
    override fun setPreferenceConfig(tenantId: String?, showOptOutChannels: Boolean) {
        SSPreferenceInternal.tenantId = tenantId
        SSPreferenceInternal.showOptOutChannels = showOptOutChannels
    }

    override fun registerCallback(preferenceCallback: PreferenceCallback) {
        SSPreferenceInternal.preferenceCallback = preferenceCallback
    }

    override fun unRegisterCallback() {
        SSPreferenceInternal.preferenceCallback = null
    }

    override fun fetchUserPreference(fetchRemote: Boolean): Response<PreferenceData> {
        val response = SSPreferenceInternal.fetchAndSavePreferenceData(fetchRemote)
        if (fetchRemote)
            sendUpdate()
        return response
    }
    ...
    override fun fetchCategories(
        ...
        val response = SSPreferenceInternal.fetchCategories(limit, offset)
        return response
    }
    ...
    override fun fetchCategory(
        ...
        val response = SSPreferenceInternal.fetchCategory(category)
        sendUpdate()
        return response
    }

    override fun fetchOverallChannelPreferences(): Response<JSONObject> {
        val response = SSPreferenceInternal.fetchOverallChannelPreferences()
        sendUpdate()
        return response
    }
    ...
    override fun updateCategoryPreference(
        ...
        val job = CoroutineScope(Dispatchers.IO).launch(start = CoroutineStart.LAZY) {
            ...
            response = SSPreferenceInternal.updateCategoryPreference(category, preference)
        }
        val scheduleJob = job.executeWithThrottleLast("Category$category", 2000, "$category ${preference.name}")

        Logger.v(SSConstants.TAG_SUPRSEND, "isCancelled: $category : ${preference.name} : ${scheduleJob.isCancelled}")
        Logger.v(SSConstants.TAG_SUPRSEND, "isActive: $category : ${preference.name} : ${scheduleJob.isActive}")
        Logger.v(SSConstants.TAG_SUPRSEND, "isCompleted: $category : ${preference.name} : ${scheduleJob.isCompleted}")
        ...
        Logger.v(SSConstants.TAG_SUPRSEND, "Response Received : $category : ${preference.name} : ${returnResponse.getData()} : ${returnResponse.getException()?.message}")
    ...
    override fun updateChannelPreferenceInCategory(
        ...
        Logger.i(SSConstants.TAG_SUPRSEND, "updateChannelPreferenceInCategory : $category : $channel : ${preference.name}")
        if (!NetworkInfo.isConnected()) {
            sendUpdate()
            return Response.Error(NoInternetException())
        }
        var response: Response<JSONObject>? = null
        val job = CoroutineScope(Dispatchers.IO).launch(start = CoroutineStart.LAZY) {
            response = SSPreferenceInternal.updateChannelPreferenceInCategory(category, channel, preference)
        }
        val scheduleJob = job.executeWithThrottleLast("Category$category", 2000, "$category : $channel : ${preference.name}")
    ...
    override fun updateOverallChannelPreference(
        ...
        var response: Response<JSONObject>? = null
        val job = CoroutineScope(Dispatchers.IO).launch(start = CoroutineStart.LAZY) {
            response = SSPreferenceInternal.updateOverallChannelPreference(channel, channelPreferenceOptions)
        }
    ...
    private fun sendUpdate() {
        SSPreferenceInternal.preferenceCallback?.onUpdate(fetchUserPreference(false).getData() ?: PreferenceData())
    }
