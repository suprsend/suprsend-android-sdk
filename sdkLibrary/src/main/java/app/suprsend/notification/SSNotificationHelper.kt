...
object SSNotificationHelper {
    ...
    private fun showRawNotification(context: Context, rawNotification: RawNotification, pushVendor: String? = null) {
        try {
            ...
            val isShown = LocalStorage.getValue(showNotificationId)?.toBoolean()
            Logger.i("notification", "Notification isShown : ${rawNotification.notificationGroupId} $isShown")
            LocalStorage.setValue(showNotificationId, "true")
                                ...
                                .getBitmapFromUrl(
                                    UrlUtils
                                        .createNotificationLogoImage(
                                            largeIconUrl,
                                            200,
                                            UrlUtils.calculateQuality(NetworkInfo.getNetworkType())
                                        )
                                )
                        )
            }
                        ...
                        .getBitmapFromUrl(
                            UrlUtils
                                .createNotificationBannerImage(
                                    bigPictureUrl,
                                    DeviceInfo.getDeviceWidthPixel(),
                                    UrlUtils.calculateQuality(NetworkInfo.getNetworkType())
                                )
                        )
                )
...
private fun String?.getRawNotification(): RawNotification {
    ...
    if (BuildConfig.DEBUG) {
        Logger.i("notification", "Payload : $this")
    }
