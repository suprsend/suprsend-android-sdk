...
private val phoneNumberUtils: PhoneNumberUtil by lazy { PhoneNumberUtil.getInstance() }
internal fun isMobileNumberValid(mobile: String): Boolean {
    return if (mobile.contains("+"))
        phoneNumberUtils.isValidNumber(phoneNumberUtils.parse(mobile, null))
    else
        false
}

internal fun Context.runOnUIThread(task: () -> Unit) {
