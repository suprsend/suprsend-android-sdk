...
2. If using JWT mode, configure `SuprSend.setRefreshTokenCallback(...)` — the same JWT flow applies as for events and inbox.
...
Preferences use the same base URL as events (`SuprSend.initialize` → `host`):
